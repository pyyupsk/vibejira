...
const getIssue = async (issueIdOrKey, options = {}) => {
  try {
    ...
    console.error(
      `JIRA API Error (getIssue ${issueIdOrKey}):`,
      error.response ? error.response.data : error.message,
    )
...
const getIssueComments = async (issueIdOrKey) => {
  try {
    ...
    console.error(
      `JIRA API Error (getIssueComments ${issueIdOrKey}):`,
      error.response ? error.response.data : error.message,
    )
...
const addIssueComment = async (issueIdOrKey, commentBody) => {
  try {
    ...
    console.error(
      `JIRA API Error (addIssueComment ${issueIdOrKey}):`,
      error.response ? error.response.data : error.message,
    )
...
const updateIssue = async (issueIdOrKey, updatePayload) => {
  try {
    ...
    console.error(
      `JIRA API Error (updateIssue ${issueIdOrKey}):`,
      error.response ? error.response.data : error.message,
    )
